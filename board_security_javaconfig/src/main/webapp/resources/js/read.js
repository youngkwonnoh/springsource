...
$(function() {
	...
	var modalReomveBtn = $("modalReomveBtn");
	
	$("#addReplyBtn").click(function(){
		// input 안에 들어있는 value 제거
		modal.find("input").val("");
		
		// 로그인 사용자 댓글 작상자란에 아이디 보여주기
		modalReplyer.val(replyer);
		
		// 작성날짜, 수정, 삭제 버튼 제거한 후 모달 보여주기
	...
	function showReplyPage(total){
		...
		if(next){
			str += "<li class='page-item'><a class='page-link' href='" + (endPage+1) + "'>Prev</a></li>";
		}
	...
	// 이벤트 위임 : li 태그는 나중에 생기는 요소이기 때문에 ul에 먼저 이벤트를 건 후 li에 넘겨주는 방식
